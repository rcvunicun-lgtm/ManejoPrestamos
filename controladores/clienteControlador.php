...
if($peticionAjax){
	...
	require_once SERVERPATH."modelos/clienteModelo.php";
	// require_once "../modelos/usuarioModelo.php";
	//require_once "D:/Instalados/Xampp/htdocs/Prestamos/modelos/clienteModelo.php";
...
class clienteControlador extends clienteModelo {
    ...
    public function agregar_cliente_controlador(){
		...
		if(mainModel::verificar_datos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ \( \) \. \, \# \\ \- ]{1,150}",$direccion)){
			$alerta = [
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrio un error inesperado",
				"Texto"=>"La direcciòn no coincide con el formato solicitado",
				"Tipo"=>"error"
			];
			echo (json_encode($alerta));
			exit();
		...
		echo (json_encode($alerta));
    }/* Fin del controlador agregar cliente */
	...
	public function eliminar_cliente_controlador(){
		...
		$cliente_id = mainModel::decryption($_POST['cliente_id_del']);
		$cliente_id = mainModel::limpiar_cadena($cliente_id);

		// Comprobar la existencia del cliente
		$check_cliente = mainModel::ejecutar_consulta_simple("SELECT cliente_id FROM cliente WHERE cliente_id = '$cliente_id'");
		...
		$chek_prestamos = mainModel::ejecutar_consulta_simple("SELECT cliente_id FROM prestamo WHERE cliente_id = '$cliente_id' LIMIT 1");
		if($chek_prestamos->rowCount() > 0){
			$alerta = [
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrio un error inesperado",
		...
		$eliminar_cliente = clienteModelo::eliminar_cliente_modelo($cliente_id);

		if($eliminar_cliente->rowCount() == 1){
			$alerta = [
				"Alerta"=>"recargar",
				"Titulo"=>"Cliente eliminado",
				"Texto"=>"El cliente ha sido eliminado del sistema con éxito.",
				"Tipo"=>"success"
			];
		}else{
			$alerta = [
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrio un error inesperado",
				"Texto"=>"No hemos podido eliminar el cliente del sistema. Por favor intentelo nuevamente",
				"Tipo"=>"error"
			];
		}
		echo (json_encode($alerta));
		exit();
	}  /* Fin Controlador eliminar cliente */

	/*---------- Controlador datos de clientes ----------*/
	...
	public function actualizar_cliente_controlador(){
		// Recuperar el id
		$id = mainModel::decryption($_POST['cliente_id_update']);
		$id = mainModel::limpiar_cadena($id);

		//Comprobar la existencia del cliente en la base de datos
		...
		if($check_cliente->rowCount() <= 0){
			...
			if(clienteModelo::actualizar_cliente_modelo($datos_cliente_update)){
				$alerta = [
					"Alerta"=>"recargar",
					"Titulo"=>"Cliente actualizado",
					"Texto"=>"Los datos del cliente se actualizaron con éxito.",
					"Tipo"=>"success"
				];
				...
				$alerta = [
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrio un error inesperado",
					"Texto"=>"No hemos podido actualizar los datos del cliente, por favor intentelo nuevamente.",
					"Tipo"=>"error"
				];
			}
			echo (json_encode($alerta));
